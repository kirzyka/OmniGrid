...
export function APINav() {
    return (
        <aside className="px-8 py-15  max-md:px-5 max-md:py-10">
            ...
            <h1 className="mb-13.75 text-[36px] font-normal leading-[.9] tracking-[-.045em] text-ink dark:text-ink-dark max-sm:mb-5.5 max-sm:text-5xl">API</h1>
            <nav className="grid gap-2 max-sm:flex max-sm:flex-wrap max-sm:gap-x-4.5" aria-label="API sections">
                {API_ITEMS.map(([number, title, href]) => (
                    <a className="flex items-center gap-4 py-3 font-sans text-sm hover:text-mint dark:hover:text-mint-dark" href={href} key={href}>
